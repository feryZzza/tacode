...
for seed in $SEEDS; do
	...
	read -r window stride profile side limit_trials < <("$PY" - "$src" <<'EOF'
import json, sys
a = json.loads(open(sys.argv[1]).read())["args"]
print(a["window_size"], a["stride"], a["input_profile"], a["side"], a.get("limit_trials", 0))
EOF
	)
	echo "[seed $seed / gpu $gpu] reselect -> $out (window=$window profile=$profile)"
	(
